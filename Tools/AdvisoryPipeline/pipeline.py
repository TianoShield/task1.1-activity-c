...
def main():
    ...
    p_best = sub.add_parser("best-effort",
                             help="Fast, UNVALIDATED PoC: one harness-generation "
                                  ...
                                  "tested result.")
    p_best.add_argument("--advisory", help="Restrict to a single GHSA id; "
    ...
    p_best.add_argument("--limit", type=int,
                         help="Cap how many advisories to process (only "
                              "relevant when --advisory is omitted)")
    p_best.add_argument("--all", action="store_true",
                         help="Run against every advisory scan() finds, "
                              "not just the ones marked 'generate'. "
                              "'already_covered'/'unresolvable' advisories "
                              "return instantly with no LLM call. "
                              "'no_edk2_files' advisories DO make one LLM "
                              "call — an independent second opinion on "
                              "whether the deterministic path-parser missed "
                              "a real target, since it's pure regex against "
                              "known EDK2 naming suffixes and can be wrong. "
                              "Without this flag, only 'generate'-marked "
                              "advisories run.")
    p_best.add_argument("--force", action="store_true",
                         help="Generate a harness even for a module that "
                              "already has a real, hand-written HBFAplus "
                              "harness — for directly comparing our "
                              "LLM-generated output against the real one, "
                              "rather than skipping it as 'already_covered'.")
        ...
        for ghsa_id in targets_to_run:
            ...
            try:
                result = best_effort.run_best_effort(
                    ghsa_id, args.edk2_checkout, args.out_dir_base,
                    token, args.codeql_db, force_regenerate=args.force,
                )
                result_dict = dataclasses.asdict(result)
            except Exception as e:
                result_dict = {"ghsa_id": ghsa_id, "verdict": "error", "reason": str(e)}
            results.append(result_dict)
            # Write incrementally so a long multi-advisory run isn't lost
            # if it gets interrupted partway through.
            report_path = os.path.join(args.out_dir_base, "best_effort_run_report.json")
            with open(report_path, "w") as f:
                json.dump(results, f, indent=2)

        passed = [r for r in results if r.get("verdict") == "unvalidated_best_effort"]
        print(f"\n[i] Done. {len(passed)}/{len(results)} advisories cleared "
              f"the deterministic gates (verdict: unvalidated_best_effort). "
              f"Full report: {os.path.join(args.out_dir_base, 'best_effort_run_report.json')}",
              file=sys.stderr)
        if passed:
            print("[i] Passed:", [r["ghsa_id"] for r in passed], file=sys.stderr)
