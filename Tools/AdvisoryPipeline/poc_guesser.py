...
_GUESS_HEADING_RE = re.compile(r"GUESS\s*\(hex\)\s*:?", re.IGNORECASE)
# Tier 1: a hex-dump-style line, e.g. "00000000: 52 69 67 68 74 ..." — some
# models (confirmed in production, llama3.1:8b via a different endpoint)
# default to this familiar xxd-style format instead of a plain continuous
# hex string, even when asked for the latter.
_HEXDUMP_LINE_RE = re.compile(
    r"^[0-9a-fA-F]{4,}:\s*((?:[0-9a-fA-F]{2}\s+)+[0-9a-fA-F]{2})", re.MULTILINE)
# Tier 2: a plain contiguous run of hex byte pairs (the originally requested
# format), tolerant of markdown code-fence lines around it.
_PLAIN_HEX_RE = re.compile(r"([0-9a-fA-F]{2}(?:\s*[0-9a-fA-F]{2})+)")
...
def parse_guess(response: str) -> bytes:
    heading_m = _GUESS_HEADING_RE.search(response)
    if not heading_m:
        raise ValueError(
            "LLM guess response didn't contain a 'GUESS (hex):' heading; "
            f"got:\n{response[:500]}"
        )
    tail = response[heading_m.end():]
    # Stop at the REASONING section (if present) so we don't accidentally
    # scoop up hex-looking substrings from the reasoning prose itself.
    reasoning_idx = tail.upper().find("REASONING")
    if reasoning_idx != -1:
        tail = tail[:reasoning_idx]

    # Tier 1: hex-dump-style lines (address: bytes...).
    dump_matches = _HEXDUMP_LINE_RE.findall(tail)
    if dump_matches:
        hex_str = "".join(re.sub(r"\s+", "", m) for m in dump_matches)
    else:
        # Tier 2: a plain hex blob, tolerant of markdown code-fence marker
        # lines (```hex, ```) and bare bold-marker lines (**) sitting
        # around it.
        cleaned_lines = [
        ...
        candidate = "\n".join(cleaned_lines)
        plain_m = _PLAIN_HEX_RE.search(candidate)
        if not plain_m:
            raise ValueError(
                "LLM guess response had a 'GUESS (hex):' heading but no "
                f"parseable hex bytes after it; got:\n{response[:500]}"
            )
        hex_str = re.sub(r"\s+", "", plain_m.group(1))
...
def propose_single_guess(advisory: dict, target: dict, harness_source: str,
                          usage_context: str = "(not available)",
                          source_snippets: str = "(not available)"):
    """One-shot version of guess_and_test_loop, for best-effort mode: asks
    the LLM for exactly one candidate exploit and returns it immediately —
    no replay_input() call, no crash check, no build required at all.
    Purely LLM reasoning, exactly as fast (and exactly as unverified) as a
    single LLM call. Returns (winning_bytes: bytes, reasoning: str); raises
    on a parse failure rather than silently retrying, since best-effort
    mode is explicitly meant to be fast — one shot, report honestly if it
    didn't work, not another refinement loop.
    """
    import harness_generator  # reuse the one call_llm() implementation

    prompt = build_guess_prompt(advisory, target, harness_source, [],
                                 usage_context, source_snippets)
    response = harness_generator.call_llm(
        prompt,
        system_prompt=f"You are proposing a concrete exploit for a fuzz "
                       f"harness targeting {target.get('module', 'a UEFI driver')}. "
                       f"Respond only in the exact requested format.",
    )
    guessed_bytes = parse_guess(response)
    reasoning_m = re.search(r"REASONING:\s*\n?(.+)", response, re.DOTALL)
    reasoning = reasoning_m.group(1).strip()[:300] if reasoning_m else ""
    return guessed_bytes, reasoning
