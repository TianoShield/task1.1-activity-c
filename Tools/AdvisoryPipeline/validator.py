...
class ValidationResult:
    ...
    verdict: str   # "confirmed" | "no_repro" | "build_failed" | "not_regression_tested" | "false_positive"
    reason: str
...
def _add_worktree(edk2_checkout: str, commit: str, worktree_dir: str) -> None:
    ...
    # `git worktree add` checks out the tree at `commit`, including each
    # submodule's gitlink (its pinned commit reference) — but it does NOT
    # populate the submodule's actual file content, even if the main
    # checkout's own submodules are already initialized elsewhere. That
    # population step has to be run separately, from inside this specific
    # new worktree directory. Skipping this causes real build failures for
    # any target whose package transitively depends on a submodule (found
    # in production: MdePkg -> MdePkg.dec -> MipiSysTLib/mipisyst).
    #
    # Each submodule is updated in ITS OWN separate git call, deliberately
    # not one blanket `git submodule update --init` for all of them —
    # confirmed in production that a blanket call can still leave a
    # needed submodule (MipiSysTLib) unpopulated even after a DIFFERENT,
    # unrelated submodule (SubhookLib, pointed at a dead GitHub repo)
    # fails partway through, for reasons not fully diagnosable from git's
    # own combined log output. Doing this per-submodule guarantees one
    # submodule's failure can never possibly affect any other's outcome,
    # and gives real, untruncated visibility into exactly which ones
    # succeeded or failed.
    list_result = subprocess.run(
        ["git", "config", "-f", os.path.join(worktree_dir, ".gitmodules"),
         "--get-regexp", r"submodule\..*\.path"],
        check=False, capture_output=True, text=True,
    )
    submodule_paths = [line.split(" ", 1)[1] for line in list_result.stdout.splitlines() if " " in line]
    ...
    for sub_path in submodule_paths:
        try:
            ...
            # Same best-effort treatment as any other submodule failure:
            # a slow clone of a submodule this target may not even need
            # should not abort the entire evaluation with an unhandled
            # exception (confirmed in production: CryptoPkg's mbedtls
            # timed out and killed one advisory's whole run).
            failed.append(f"{sub_path} (timeout)")
    if failed:
        # Still best-effort, not fatal — a submodule this specific target
        # doesn't actually need failing shouldn't block the build. If a
        # submodule that IS needed is still missing, that surfaces as its
        # own clear, specific build_failed error (as already confirmed for
        # MipiSysTLib) — that's the real signal to act on.
        print(f"[!] {len(failed)} submodule(s) failed to update in "
              f"{worktree_dir} (continuing anyway — a submodule this "
              f"target actually needs will surface as its own specific "
              f"build error if still missing): {failed}")
...
def build_harness(inf_path: str, workspace: str, toolchain: str = "AFL",
    ...
    return BuildResult(ok=ok, binary_path=binary_path, log_tail=log[-50000:])
...
def validate(edk2_checkout: str, pre_fix_commit: str, fix_commit: str,
             ...
             duration_s: int = 120, direct_guess_fn=None,
             use_afl_fallback: bool = True,
             sanity_check_fn: Optional[Callable[["FuzzResult", "FuzzResult"], tuple]] = None,
             ) -> ValidationResult:
    ...
    AFL++ (or straight to no_repro, if use_afl_fallback is False).

    `use_afl_fallback` — PoCGen has NO blind-search/fuzzing step anywhere
    in its architecture: it's 100% LLM-guess -> execute once -> refine with
    feedback -> guess again. Set this False to match that exactly: if
    direct_guess_fn's guesses all miss, the attempt is simply no_repro,
    full stop — AFL++'s run_short_fuzz() never runs at all. True (the
    default) keeps AFL++ as HBFAplus's own safety net beyond what PoCGen
    itself does, for bugs a few guesses can't find directly.
    """
    ...
    try:
        ...
        if pre_fuzz is None and use_afl_fallback:
            pre_fuzz = run_short_fuzz(pre_build.binary_path, seed_dir, duration_s)
        elif pre_fuzz is None:
            # PoCGen-exact mode: no blind search, no AFL++ call at all —
            # guessing missed, this attempt is simply a miss.
            pre_fuzz = FuzzResult(
                crashed=False, crash_input=None, asan_summary=None,
                stderr_tail="(AFL++ fallback disabled — direct guessing "
                            "found no crash within its attempt budget)",
            )
        ...
        if not post_build.ok:
            return ValidationResult(
                pre_fix=pre_fuzz,
                post_fix=FuzzResult(False, None, None, post_build.log_tail),
                verdict="not_regression_tested",
        ...
        if sanity_check_fn is not None:
            passed, reasoning = sanity_check_fn(pre_fuzz, post_fuzz)
            if not passed:
                return ValidationResult(
                    pre_fix=pre_fuzz, post_fix=post_fuzz, verdict="false_positive",
                    reason="Crash reproduces pre-fix and is absent post-fix, but "
                           "the LLM sanity check determined this does not "
                           "genuinely demonstrate the advisory's described "
...
def _prepare_workspace(edk2_worktree: str, hbfaplus_source: str) -> str:
    ...
    # Unaligned.c ABI-compat shim: BaseLib.h's ReadUnaligned16/etc. prototypes
    # changed from fixed-width pointer types to VOID* in commit 2ba7e5be6c.
    # Pick whichever HBFAplus/HostLib/BaseLibHost/Unaligned.c variant matches
    # this worktree's checked-out BaseLib.h, since HBFAplus/ is a symlink
    # shared across worktrees at different edk2 revisions.
    baselib_h = os.path.join(edk2_worktree, "MdePkg", "Include", "Library", "BaseLib.h")
    unaligned_dir = os.path.join(hbfaplus_source, "HostLib", "BaseLibHost")
    unaligned_target = os.path.join(unaligned_dir, "Unaligned.c")
    old_style = os.path.join(unaligned_dir, "Unaligned.c.old_style")
    new_style = os.path.join(unaligned_dir, "Unaligned.c.new_style")

    if os.path.exists(baselib_h) and os.path.exists(old_style) and os.path.exists(new_style):
        with open(baselib_h) as f:
            baselib_text = f.read()
        m = re.search(r"ReadUnaligned16\s*\(\s*\n\s*IN CONST (\w+)\s*\*Buffer", baselib_text)
        wants_void = m and m.group(1) == "VOID"
        chosen = new_style if wants_void else old_style
        with open(chosen) as f:
            chosen_text = f.read()
        current_text = None
        if os.path.exists(unaligned_target):
            with open(unaligned_target) as f:
                current_text = f.read()
        if current_text != chosen_text:
            shutil.copyfile(chosen, unaligned_target)
